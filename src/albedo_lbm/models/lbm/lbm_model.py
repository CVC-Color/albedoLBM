...
class LBMModel(BaseModel):
    ...
    def __init__(
        ...
        self.recons_loss_type = config.recons_loss_type
    ...
    def forward(self, batch: Dict[str, Any], step=0, batch_idx=0, *args, **kwargs):
        ...
        if self.mask_key in batch:
            ...
            valid_mask = torch.ones_like(batch["albedo"]).bool()
        ...
        if self.alpha >= 0:
            ...
            #Pixel loss
            shading = batch.get("shading", None)
            residual = batch.get("residual", None)
            shading_detached = shading.detach() if shading is not None else None
            residual_detached = residual.detach() if residual is not None else None
            image = batch["image"]
            pixel_loss, reconstruction_loss = self.pixel_and_reconstruction_loss(
                denoised_sample,
            ...
            reconstruction_loss = torch.zeros_like(latent_recon_loss)
    ...
    alpha,
        ...
        input_crop_h = max((image.shape[2] - self.pixel_loss_max_size), 0)
        input_crop_w = max((image.shape[3] - self.pixel_loss_max_size), 0)

        offset_h = torch.randint(0, crop_h, (1,)).item() if crop_h > 0 else 0
        offset_w = torch.randint(0, crop_w, (1,)).item() if crop_w > 0 else 0
        ...
        # ---- crop image-space tensors ----
        def crop_input(x):
            return x[
                :,
                :,
                input_crop_h - input_offset_h : input_crop_h - input_offset_h + input_crop,
                input_crop_w - input_offset_w : input_crop_w - input_offset_w + input_crop,
            ]

        target_pixels = crop_input(target_pixels)
        image = crop_input(image)
        if shading is not None:
            shading = crop_input(shading)
        if residual is not None:
            residual = crop_input(residual)
        valid_mask = crop_input(valid_mask)

        # 🔥 decode only once
        decoded_prediction = self.vae.decode(prediction).clamp(-1, 1)

        # =====================================================
        # PIXEL LOSS (albedo supervision)
        # =====================================================
        if alpha > 0:
            if self.pixel_loss_type == "l2":
                pixel_loss = torch.mean(
                    ((decoded_prediction * valid_mask - target_pixels * valid_mask) ** 2)
                    .reshape(image.shape[0], -1),
                    1,
                )
            elif self.pixel_loss_type == "l1":
                pixel_loss = torch.mean(
                    torch.abs(decoded_prediction * valid_mask - target_pixels * valid_mask)
                    .reshape(image.shape[0], -1),
                    1,
                )
        ...
        # =====================================================
        if alpha < 1:
            if shading is None or residual is None:
            ...
            elif self.recons_loss_type == "l1":
                reconstruction_loss = torch.mean(
                    torch.abs(reconstructed * valid_mask - image * valid_mask)
                    .reshape(image.shape[0], -1),
                    1,
                )
            elif self.recons_loss_type == "lpips":
                reconstruction_loss = self.lpips_loss(
                    reconstructed * valid_mask,
                    image * valid_mask,
                ).mean()
        if alpha == 1:
             reconstruction_loss = torch.zeros_like(pixel_loss)
        if alpha == 0:
            pixel_loss = torch.zeros_like(reconstruction_loss)
        return pixel_loss, reconstruction_loss
